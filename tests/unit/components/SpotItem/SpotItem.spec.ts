...
describe('components/SpotItem.vue', () => {
    ...
    it('対象スポットの親マップに親スポットが存在する場合' , () => {
        ...
        expect(wrapper.find('.v-list-item__title').text()).toBe(parentSpot.getName() + ' ' + testSpotWithFloorName.getName());
        expect(wrapper.find('.v-list-item__subtitle').text()).toBe(testSpotWithFloorName.getFloorName() + ' ' + '1km');
    })
});
