...
import { RawMapData, RawSpotData, DisplayLevelType } from '@/store/types';
...
export class MapViewMutations extends Mutations<MapViewState> {
    ...
    public setLastViewedDetailMapId(
        ...
        const mapIndex: number = this.state.maps.findIndex((m: RawMapData) => m.id === parentMapId);
        const spotIndex: number = this.state.maps[mapIndex].spots.findIndex((s: RawSpotData) => s.id === spotId);
