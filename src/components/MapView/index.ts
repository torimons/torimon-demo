...
import { SpotForMap, Coordinate, Bounds, RawSpotData, DisplayLevelType } from '@/store/types';
...
export default class MapView extends Vue {
    ...
    private updateIdOfCenterSpotInRootMap(e: L.LeafletEvent): void {
        ...
        const spots: RawSpotData[] = mapViewGetters.maps[mapIndex].spots;
     ...
     */
    private getNearestSpotId(basePoint: Coordinate, spots: RawSpotData[]): number {
        const spotPositions: Coordinate[] = spots.map((s: RawSpotData) => s.coordinate);
        const nearestSpotPos: GeolibInputCoordinates = findNearest(basePoint, spotPositions);
        const nearestSpotIndex: number = spots.findIndex((s: RawSpotData) => s.coordinate === nearestSpotPos);
    ...
    private watchStoreForMoveMapCenter(): void {
        store.watch(
            ...
            (spot, oldSpot) => {
                ...
                const spotToDisplayInMapCenter: RawSpotData
