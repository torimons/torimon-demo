...
import { RawMapData, RawSpotData } from '@/store/types';
...
export default class SpotSearch extends Vue {
    ...
    private spotSearchResults: RawSpotData[] = [];
    private search!: Search;
    private backgroundColor: 'transparent' | 'white' = 'transparent';

    public mounted() {
        // 全てのマップからスポットを取得，一つの配列に結合する
        mapViewGetters.maps.map((map: RawMapData) => map.spots)
            .forEach((spots: RawSpotData[]) => this.targetSpots = this.targetSpots.concat(spots));
